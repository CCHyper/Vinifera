...
ClientGameSettingsType Client::GameSettings;
...
static void Client_Debug_Log_Settings()
{
    ...
    for (int i = 0; i < Client::GameSettings.Players.Count(); ++i) {
        ClientPlayerSettingsType &player = *Client::GameSettings.Players[i];
...
bool Client::Parse_Command_Line(int argc, char *argv[])
{
    ...
    for (int index = 1; index < argc; index++) {
        ...
        *dest++ = '\0';
        ...
        /**
*  Format: "-CLIENT=<settings>,<map>,<mode>,<gameid>"
         *  Format: "-CLIENT:1,1,1"
         */
        if (std::strstr(string, "-CLIENT")) {
            ...
            IsRunFromClientOnly = std::strtol(tmpstr, nullptr, 10) == 1 ? true : false;
            ...
            Client::SettingsFile.Set_Name(settingsstr);
            if (!Client::SettingsFile.Is_Available()) {
            ...
            Client::SettingsFile.Open();
            Client::SettingsINI.Load(Client::SettingsFile, false);

            char *mapstr = std::strtok(nullptr, ",");
            Client::ScenarioFile.Set_Name(mapstr);
            if (!Client::ScenarioFile.Is_Available()) {
                DEBUG_ERROR(TXT_CLIENT_INALIDSCENARIO);
                ShowCursor(TRUE);
                MessageBox(nullptr, TXT_CLIENT_RUN_ERROR, "Vinifera", MB_OK);
                Fatal(TXT_CLIENT_INALIDSCENARIO);
            }
            Client::ScenarioFile.Open();
            Client::ScenarioINI.Load(Client::ScenarioFile, false);
            ...
            Client::ExitOnGameFinish = (std::strtol(modestr, nullptr, 10) == 1) ? true : false;
            ...
            Client::GameID = std::strtol(idstr, nullptr, 10);

            /**
             *  Load the settings now before any other initialisation is done.
             */
            bool loaded = Client::Read_Settings_INI(Client::SettingsINI);
            if (!loaded) {
            ...
            //DEBUG_INFO("Client: %s %s %d %d\n", Client::SettingsFile.File_Name(), Client::ScenarioFile.File_Name(), Client::ExitOnGameFinish, Client::GameID);
...
bool Client::Read_CnCNet_Settings(CCINIClass &ini)
{
    ...
    if (!ini.Is_Present(CNCNET)) {
        DEBUG_ERROR("Client: Failed to find CnCNet settings!\n");
        return false;
    }
...
bool Client::Read_Client_Settings(CCINIClass &ini)
{
    ...
    static char const * const CLIENT = "Client";
    if (!ini.Is_Present(CLIENT)) {
        DEBUG_ERROR("Client: Failed to find Client settings!\n");
        return false;
    }

#ifndef NDEBUG
    IsRunFromClientOnly = ini.Get_Bool(CLIENT, "RunFromClientOnly", IsRunFromClientOnly);
    IsExitOnGameFinish = ini.Get_Bool(CLIENT, "ExitOnGameFinish", IsExitOnGameFinish);
    IsSkipStartupMovies = ini.Get_Bool(CLIENT, "SkipStartupMovies", IsSkipStartupMovies);
#endif
...
bool Client::Read_Game_Settings(CCINIClass &ini)
{
    ...
    GameSettings.IsMultiplayerGame = ini.Get_Bool(SETTINGS, "MultiplayerGame", false);
    GameSettings.IsNormalGame = ini.Get_Bool(SETTINGS, "NormalGame", false);
    GameSettings.IsSkirmishGame = ini.Get_Bool(SETTINGS, "SkirmishGame", true);

    GameSettings.IsFirestormAddon = ini.Get_Bool(SETTINGS, "Firestorm", false);

    /**
     *  Load the campaign type.
     */
    if (GameSettings.IsNormalGame) {
        GameSettings.Campaign = (CampaignType)ini.Get_Int(SETTINGS, "Campaign", CAMPAIGN_NONE);
    }
     ...
     */
    if (!GameSettings.IsNormalGame) {
        GameSettings.IsShortGame = ini.Get_Bool(SETTINGS, "ShortGame", false);
        GameSettings.IsMultiEngineer = ini.Get_Bool(SETTINGS, "MultiEngineer", false);
        GameSettings.IsMCVRedeploy = ini.Get_Bool(SETTINGS, "MCVRedeploy", false);
        GameSettings.IsCrateGoodies = ini.Get_Bool(SETTINGS, "CrateGoodies", false);
        GameSettings.IsBridgeDestruction = ini.Get_Bool(SETTINGS, "BridgeDestruction", false);
        GameSettings.IsTiberiumGrows = ini.Get_Bool(SETTINGS, "TiberiumGrows", false);
        GameSettings.IsFogOfWar = ini.Get_Bool(SETTINGS, "FogOfWar", false);
        GameSettings.IsShroudRegrows = ini.Get_Bool(SETTINGS, "ShroudRegrows", false);
        GameSettings.IsHarvesterTruce = ini.Get_Bool(SETTINGS, "HarvesterTruce", false);
        GameSettings.IsAlliesAllowed = ini.Get_Bool(SETTINGS, "AlliesAllowed", false);
        GameSettings.IsFixedAlliance = ini.Get_Bool(SETTINGS, "FixedAlliance", false);

        GameSettings.Credits = ini.Get_Int(SETTINGS, "Credits", 0);
        GameSettings.TechLevel = ini.Get_Int(SETTINGS, "TechLevel", 0);
        GameSettings.UnitCount = ini.Get_Int(SETTINGS, "UnitCount", 0);
        GameSettings.PlayerCount = ini.Get_Int(SETTINGS, "Players", 0);
        GameSettings.AIPlayerCount = ini.Get_Int(SETTINGS, "AIPlayers", 0);
    }
     ...
     */
    if (GameSettings.IsMultiplayerGame) {
        GameSettings.Seed = ini.Get_Int(SETTINGS, "Seed", 0);
    } else {
    ...
    if (!GameSettings.IsNormalGame) {
        ...
        for (int i = 0; i < total_players; ++i) {
            ...
            ClientPlayerSettingsType *player = new ClientPlayerSettingsType;
             ...
             */
            player->IsHuman = ini.Get_Bool(buffer, "Human", false);

            /**
             *  Check to make sure no other human players have been set.
             */
            ...
            if (GameSettings.IsMultiplayerGame) {
                 ...
                 */
                player->IsLocal = ini.Get_Bool(buffer, "Local", false);

                /**
                 *  Is this player the host of the game?
                 */
                player->IsHost = ini.Get_Bool(buffer, "Host", false);
             ...
             *  
             */
            player->IsPlayerControl = ini.Get_Bool(buffer, "PlayerControl", false);

            /**
             *  Set the difficult for the AI houses.
             */
            if (!player->IsHuman) {
                player->Difficulty = (DiffType)ini.Get_Int(buffer, "Difficulty", DIFF_EASY);
            } else {
            ...
            if (!stricmp(waypt_buffer, "<random>")) {
                ...
                player->StartLocation = (WaypointEnum)ini.Get_Int(buffer, "StartWaypoint", -1);
             ...
             *  
             */
            player->Side = ini.Get_SideType(buffer, "Side", SIDE_FIRST);
            ...
            if (!stricmp(house_buffer, "<random>")) {
                ...
                player->House = ini.Get_HousesType(buffer, "House", HOUSE_NONE);
            ...
            if (!stricmp(house_buffer, "<random>")) {
                ...
                player->ColorScheme = ini.Get_ColorSchemeType(buffer, "Color", COLORSCHEME_NONE);
             ...
             *  
             */
            player->Team = (ClientTeamType)ini.Get_Int(buffer, "Team", -1);
            if (player->Team == -1 || player->Team >= TEAM_COUNT) {
        ...
        for (int i = 0; i < GameSettings.Players.Count(); ++i) {
            ...
            for (int j = 0; j < GameSettings.Players.Count(); ++j) {
                ...
                ClientPlayerSettingsType &existing_player = *GameSettings.Players[j];
...
bool Client::Init()
{
    RawFileClass file("CLIENT_GAME.INI");
    CCINIClass ini;
     ...
     */
    if (!Read_CnCNet_Settings(ini)) {
        return false;
    }
    if (!Read_Client_Settings(ini)) {
        return false;
    }
    if (!Read_Game_Settings(ini)) {
        return false;
    }
...
{
#ifdef NDEBUG
    RawFileClass("CLIENT_GAME.INI").Delete();
#endif
    return true;
}
...
bool Client::Assign_Houses()
{
    ...
    for (int i = 0; i < Session.Players.Count(); ++i) {
        ...
        ClientPlayerSettingsType &client_player = *GameSettings.Players[i];
         ...
         */
        if ((Client::GameSettings.IsMultiplayerGame && client_player.IsLocal)
          ||(Client::GameSettings.IsSkirmishGame && i == 0)) {
    ...
    for (int i = 0; i < Session.Players.Count(); ++i) {
        ...
        while (ally_name) {
            ...
            for (int j = 0; j < GameSettings.Players.Count(); ++j) {
                ...
                ClientPlayerSettingsType &with_player = *GameSettings.Players[j];
                ...
                if (!std::strcmp(ally_name, with_player.IniName)) {
                    ...
                    //DEBUG_INFO("Client: Player \"%s\" allied with \"%s\".\n",
                    //    client_player.IniName, with_player.IniName);
                    DEBUG_INFO("  Player \"%s\" allied with \"%s\".\n",
...
const char *Client::Name_From_Team(ClientTeamType team)
{
    ...
    return team < TEAM_COUNT ? _team_names[team] : "<invalid>";
}
