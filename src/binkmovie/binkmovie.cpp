 ...
 */
Wstring BinkMoviePlayer::BinkSearchPath;
 ...
 *  @author: CCHyper
 */
BinkMoviePlayer::BinkMoviePlayer(const Wstring &filename, bool can_be_stretched) :
    IsBreakoutAllowed(true),
    BinkHandle(nullptr),
    BinkBuffer(nullptr),
    Filename(),
 ...
 *  @author: CCHyper
 */
bool BinkMoviePlayer::Open(const Wstring &filename)
{
    Close();
     ...
     */
    const Wstring sub_filename = BinkSearchPath + filename;
    if (RawFileClass(sub_filename.Peek_Buffer()).Is_Available()) {
        BinkHandle = BinkOpen(sub_filename.Peek_Buffer(), 0);

    /**
     *  Open a handle to the file if it exists locally.
     */
    } else if (RawFileClass(filename.Peek_Buffer()).Is_Available()) {
        BinkHandle = BinkOpen(filename.Peek_Buffer(), 0);
        ...
        if (MFCC::Offset(filename.Peek_Buffer(), nullptr, &mixfile, &start)) {

            /**
             *  
             */
            FileHandle = CreateFile(mixfile->Filename, GENERIC_READ, FILE_SHARE_READ|FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL|FILE_SUPPORTS_BLOCK_REFCOUNTING, nullptr);
            ...
            if (!IsIngameMovie && WWKeyboard->Check()) {
                ...
                switch (WWKeyboard->Get()) {
                    ...
                    case (KN_RLSE_BIT|KN_COMMA):
                        if (Vinifera_DeveloperMode && !IsPlaying) {
...
bool BinkMoviePlayer::Next_Frame(bool draw_frame)
{
     ...
     */
    if (FrameChanged || !BinkWait(BinkHandle)) {
...
bool BinkMoviePlayer::Resume_Pause()
{
    ...
    if (GameInFocus) {
        if (!playing) {
            DEBUG_INFO("Bink: Resume movie.\n");
            IsPlaying = true;
            Pause(false);
            Draw_Frame();
        }
    } else if (playing) {
        DEBUG_INFO("Bink: Pause movie.\n");
        IsPlaying = false;
        Pause(true);
    }
 ...
 *         absolute path like "C:/MY_MOVIES/", but it must have a trailing '//'!
 * 
 *  @author: CCHyper
 */
void BinkMoviePlayer::Set_Search_Path(const Wstring &dir_name)
{
    BinkSearchPath = dir_name;
}
