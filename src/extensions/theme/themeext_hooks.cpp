    ...
    if (Audio_Driver()->Is_Available() && !Debug_Quiet && Themes.Count() > 0) {
        for (ThemeType theme = THEME_FIRST; theme < Themes.Count(); ++theme) {
            ...
            if (!Audio_Driver_Is_Direct_Sound()) {
                ...
                if (Is_Vorbis_Available()) {
                    fname = _Theme_File_Name_Ext(theme, "xOGG"); // Ogg disabled for now.
                ...
                fname = _Theme_File_Name_Ext(theme, "xMP3"); // Mp3 disabled for now.
                Themes[theme]->Available = Is_Music_File_Available(fname.Peek_Buffer());
                if (Themes[theme]->Available) {
                    DEV_DEBUG_INFO("Theme: Found Mp3 for \"%s\".\n", tctrl->Name);
                    continue;
                }
        ...
        if (theme != THEME_NONE && theme != THEME_QUIET) {
            if (Volume > 0) {
                ...
                if (found) {
                    ...
                    Current = Audio_Driver()->Stream_File(fname, VOLUME_MAX, true);
