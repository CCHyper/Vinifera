...
static const char *MSEngine_Get_Available_Filename(const char *name)
{
    ...
    if (!Audio_Driver_Is_Direct_Sound()) {
        ...
        fname = Build_File_Name(_fnamebuf, "xOGG"); // Ogg disabled for now.
        if (Is_Sfx_File_Available(fname.Peek_Buffer())) {
            found = true;
        }

        if (!found) {
            fname = Build_File_Name(_fnamebuf, "xMP3"); // Mp3 disabled for now.
            if (Is_Sfx_File_Available(fname.Peek_Buffer())) {
                found = true;
            }
        }
        ...
        if (!found) {
            fname = Build_File_Name(_fnamebuf, "xWAV"); // Wav disabled for now.
            if (Is_Sfx_File_Available(fname.Peek_Buffer())) {
                found = true;
            }
        }
    ...
    if (!found) {
        fname = Build_File_Name(_fnamebuf, "AUD");
        if (Is_Sfx_File_Available(fname.Peek_Buffer())) {
            found = true;
        }
    }
