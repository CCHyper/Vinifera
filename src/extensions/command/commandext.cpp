...
int New_Create_Air_Reinforcement(HouseClass *house, AircraftType air, int number, MissionType mission, TARGET tarcom, TARGET navcom, DynamicVectorClass<const TechnoTypeClass *> &passengers)
{
    ASSERT(house != 0);
        ...
        */
        ...
        if (placed) {
            ...
            */
            ...
            if (obj->What_Am_I() == RTTI_AIRCRAFT && passengers.Count() > 0) {
...
//                }
                ...
                //if (inftype != nullptr) {
                    ...
                    for (int i = 0; i < passengers.Count(); ++i) {
                        FootClass *inf = (FootClass *)passengers[i]->Create_One_Of(house);
                        if (inf) {
                            inf->Limbo();
...
bool SpawnReinforcementsCommandClass::Process()
{
    ...
    TARGET tarcom = &Map[use_this_cell];
    //if (!tarcom) {
    //    return false;
    //}

    //Cell cell = Map.Nearby_Location(Coord_Cell(PlayerPtr->Center), SPEED_WINGED, -1, MZONE_FLYER);
    TARGET navcom = nullptr;// &Map[use_this_cell];
    //if (!navcom) {
    //    return false;
    //}


    //AircraftType paradrop = AircraftTypeClass::From_Name("ORCATRAN");
    AircraftType paradrop = AircraftTypeClass::From_Name("PARAPLANE");
    AircraftType spyplane = AircraftTypeClass::From_Name("ORCA");

    InfantryType passenger = InfantryTypeClass::From_Name("GHOST");
    int passenger_count = 5;

    DynamicVectorClass<const TechnoTypeClass *> passengers;
    passengers.Add(InfantryTypeClass::As_Pointer("GHOST"));
    passengers.Add(InfantryTypeClass::As_Pointer("E1"));
    passengers.Add(InfantryTypeClass::As_Pointer("E1"));
    passengers.Add(UnitTypeClass::As_Pointer("MCV"));
    passengers.Add(UnitTypeClass::As_Pointer("MMCH"));


    // paradrop
    //New_Create_Air_Reinforcement(PlayerPtr, paradrop, 1, MISSION_PARADROP_APPROACH, tarcom, navcom, passenger, passenger_count);
    New_Create_Air_Reinforcement(PlayerPtr, paradrop, 1, MISSION_PARADROP_APPROACH, tarcom, navcom, passengers);



    // spyplane
    //New_Create_Air_Reinforcement(PlayerPtr, spyplane, 1, MISSION_SPYPLANE_APPROACH, tarcom, navcom);
