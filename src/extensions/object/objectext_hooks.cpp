...
static void Create_New_Line_Trail(ObjectClass *this_ptr)
{
    ObjectClassExtension *objectext;
    ObjectTypeClass *objecttype;
    ObjectTypeClassExtension *objecttypeext;

    objectext = ObjectClassExtensions.find(this_ptr);

    objecttype = this_ptr->Class_Of();
    objecttypeext = ObjectTypeClassExtensions.find(objecttype);

    if (objecttypeext && objecttypeext->LineTrailTypes.Count() > 0) {

        for (int i = 0; i < objecttypeext->LineTrailTypes.Count(); ++i) {

            LineTrail *linetrail = new LineTrail();
            if (linetrail) {

                LineTrailType *linetrailtype = objecttypeext->LineTrailTypes[i];

                linetrail->Set_Color(linetrailtype->Color);
                linetrail->Set_Color_Decrement(linetrailtype->ColorDecrement);

                objectext->AttachedLineTrails.Add(linetrail);
            }
...
DECLARE_PATCH(_ObjectClass_Unlimbo_Create_Line_Trail_Patch)
{
    ...
    static ObjectTypeClass *objecttype;
    static ObjectTypeClassExtension *objecttypeext;
    static LineTrail *linetrail;
    ...
    if (objecttypeext && objecttypeext->IsUseLineTrail) {
        Create_New_Line_Trail(this_ptr);
    }
