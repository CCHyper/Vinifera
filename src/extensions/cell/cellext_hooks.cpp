...
DECLARE_PATCH(_CellClass_Goodie_Check_BaseUnit_Quantity_Patch)
{
    ...
    if (RulesExtension && RulesExtension->BaseUnit.Count() > 0) {
         ...
         */
        unittype = objhouse->Get_First_Ownable(RulesExtension->BaseUnit);
        if (unittype) {
...
label_1:
    _asm { mov eax, Rule } // Second dereference required due to the global reference in TS++.
    _asm { mov eax, [eax] }
...
{
    GET_REGISTER_STATIC(FootClass *, object, edi);
...
void CellClassExtension_Hooks()
{
    ...
    Patch_Jump(0x004581BA, &_CellClass_Goodie_Check_No_Buildings_Force_MCV_BaseUnit_Patch);
}
