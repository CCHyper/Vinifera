...
static Surface *_SidebarClass_StripClass_PCXImage = nullptr;

DECLARE_PATCH(_SidebarClass_StripClass_ObjectTypeClass_PCX_Cameo_Patch)
{
    ...
    _SidebarClass_StripClass_PCXImage = nullptr;

    technotypeext = TechnoTypeClassExtensions.find(reinterpret_cast<const TechnoTypeClass *>(obj));
    if (technotypeext && technotypeext->IsCameoDataPCX) {
        _SidebarClass_StripClass_PCXImage = (Surface *)technotypeext->CameoDataPCX;
    }
...
DECLARE_PATCH(_SidebarClass_StripClass_SuperWeaponType_PCX_Cameo_Patch)
{
    ...
    if (supertypeext && supertypeext->IsCameoDataPCX) {
        _SidebarClass_StripClass_PCXImage = (Surface *)supertypeext->CameoDataPCX;
    }
...
DECLARE_PATCH(_SidebarClass_StripClass_PCX_Cameos_Patch)
{
     ...
     *  
     */
    if (_SidebarClass_StripClass_PCXImage) {
        pcx_image = _SidebarClass_StripClass_PCXImage;
    }
    ...
    if (pcx_image) {
        ...
        ImageCollection.Draw(pcxrect, *SidebarSurface, *pcx_image);
     ...
     */
         ...
         */
        CC_Draw_Shape(SidebarSurface, CameoDrawer, shapefile, 0, &pointxy, window_rect, ShapeFlagsType(SHAPE_400|SHAPE_NORMAL));
    }
...
{
    Patch_Jump(0x005F5188, &_SidebarClass_StripClass_ObjectTypeClass_PCX_Cameo_Patch);
    Patch_Jump(0x005F5216, &_SidebarClass_StripClass_SuperWeaponType_PCX_Cameo_Patch);
    Patch_Jump(0x005F52AF, &_SidebarClass_StripClass_PCX_Cameos_Patch);
}
