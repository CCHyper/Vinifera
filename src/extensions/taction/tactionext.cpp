...
const char *TActionClassExtension::Action_Description(int action)
{
    ...
    switch (action) {
        ...
        case TACTION_HOUSE_DIE:
            return "The house specified will blow up after a period of time.";
...
bool TActionClassExtension::Execute(TActionClass *this_ptr, HouseClass *house, ObjectClass *object, TriggerClass *trigger, Cell *cell)
{
    ...
    switch (this_ptr->Action) {
        ...
        case TACTION_HOUSE_DIE:
            success = TAction_House_Flag_To_Die(this_ptr, house, object, trigger, cell);
            break;
 ...
 *  #issue-160
 * 
 *  Flags the house to blow up (surrender) soon.
 * 
 *  Expected INI format;
 *  NAME = [Action Count], 107, 0, [PARAM1 = House ID], 0, 0, 0, 0
 * 
 *  For FinalSun;
 *  107=Self destruct... [Requires Vinifera],0,2,0,0,0,0,0,0,0,The house specified will blow up after a period of time.,1,0,107
 * 
 *  @author: CCHyper
 */
bool TActionClassExtension::TAction_House_Flag_To_Die(TActionClass *this_ptr, HouseClass *house, ObjectClass *object, TriggerClass *trigger, Cell *cell)
{
    if (!this_ptr || !house) {
        return false;
    }
    ...
    /**
     *  Flag the house to die.
     */
#ifndef NDEBUG
    DEV_DEBUG_INFO("  Flagging house \"%s\" to die.\n", hptr->Name());
#endif
    hptr->Flag_To_Die();
