...
HRESULT DirectShowVideoPlayer::Open_File(const char *pszFileName)
{
    ...
    MultiByteToWideChar(CP_ACP, 0, dest_filename.Peek_Buffer(), -1, wFileName, MAX_PATH);
     ...
     *  Store the filename.
     */
    Filename = dest_filename.Peek_Buffer();
 ...
 *  @author: CCHyper (based on Windows SDK sample code)
 */
BOOL DirectShowVideoPlayer::Is_File_Available(const char *pszFileName, Wstring *dest)
{
    ...
    //    };
         ...
         */
        CDFileClass localfile(filename.Peek_Buffer());
