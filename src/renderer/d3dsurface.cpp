...
bool D3DSurface::Unlock()
{
    if (LockLevel > 0) {
        ...
        if (LockLevel == 0) {
            ...
            HRESULT res = VideoSurfacePtr->UnlockRect();

            if (FAILED(res)) {
                DEBUG_ERROR("D3DSurface - Failed to unlock surface! Error code %x!\n", res);
                return false;
            }
...
bool D3DSurface::Copy_From(Rect &toarea, Rect &torect, Surface &fromsurface, Rect &fromarea, Rect &fromrect, bool trans_blit, bool a7)
{
    ...
    if (fromsurface.Is_Locked() || trans_blit) {
        use_xsurface = true;
    }
    ...
    //HRESULT hr = VideoSurfacePtr->Blt(&dest_rectangle, static_cast<D3DSurface &>(fromsurface).VideoSurfacePtr, &source_rectangle, DDBLT_WAIT, nullptr);
    //return SUCCEEDED(hr);
