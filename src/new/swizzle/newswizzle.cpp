...
#define SWIZZLE_DEBUG_TRACE(x, ...) DEV_DEBUG_TRACE(x, ##__VA_ARGS__)
#else
#define SWIZZLE_DEBUG_INFO(x, ...) ((void)0)
#define SWIZZLE_DEBUG_WARNING(x, ...) ((void)0)
#define SWIZZLE_DEBUG_ERROR(x, ...) ((void)0)
#define SWIZZLE_DEBUG_FATAL(x, ...) ((void)0)
#define SWIZZLE_DEBUG_TRACE(x, ...) ((void)0)
#endif


static int __cdecl ptr_compare_function(const void *ptr1, const void *ptr2)
{
    if (static_cast<const ViniferaSwizzleManagerClass::SwizzlePointerStruct *>(ptr1)->ID < static_cast<const ViniferaSwizzleManagerClass::SwizzlePointerStruct *>(ptr2)->ID) {
        return -1;
    }
    if (static_cast<const ViniferaSwizzleManagerClass::SwizzlePointerStruct *>(ptr1)->ID > static_cast<const ViniferaSwizzleManagerClass::SwizzlePointerStruct *>(ptr2)->ID) {
        return 1;
...
/**
 *  ViniferaSwizzleManagerClass::QueryInterface
 * 
...
/**
 *  ViniferaSwizzleManagerClass::AddRef
 * 
...
/**
 *  ViniferaSwizzleManagerClass::Release
 * 
...
/**
 *  ViniferaSwizzleManagerClass::Reset
 * 
...
/**
 *  ViniferaSwizzleManagerClass::Swizzle
 * 
...
/**
 *  ViniferaSwizzleManagerClass::Fetch_Swizzle_ID
 * 
...
/**
 *  ViniferaSwizzleManagerClass::Here_I_Am
 * 
 *  tomsons26: "Here I am, rock you like a hurricane!"
 *  ...Anyways, inform (register) swizzler of new object location.
 * 
 *  @author: CCHyper
 */
...
/**
 *  ViniferaSwizzleManagerClass::Save_Interface
 * 
...
/**
 *  ViniferaSwizzleManagerClass::Load_Interface
 * 
...
/**
 *  ViniferaSwizzleManagerClass::Sort_Tables
 *  
...
void ViniferaSwizzleManagerClass::Sort_Tables()
{
    if (PointerTable.Count() > 0) {
        std::qsort(&PointerTable[0], PointerTable.Count(), sizeof(SwizzlePointerStruct), ptr_compare_function);
    }
    if (RequestTable.Count() > 0) {
        std::qsort(&RequestTable[0], RequestTable.Count(), sizeof(SwizzlePointerStruct), ptr_compare_function);
    }
}


/**
 *  ViniferaSwizzleManagerClass::Process_Tables
 *  
        ...
        while (request_count > 0) {
            ...
            if (pre_search_id == ptr_id) {
                ...
                *ptr = (uintptr_t)PointerTable[pointer_index].Pointer;
                ...
                SWIZZLE_DEBUG_INFO("SwizzleManager::Process_Tables() - Remapped \"%s\" (ID: %08X) to 0x%08X.\n",
            ...
            if (failed) {
                ...
                /**
                 *  Produce a crash dump for developer analysis.
                 */
...
/**
 *  ViniferaSwizzleManagerClass::Swizzle_Dbg
 * 
...
#ifdef SWIZZLE_DEBUG_PRINT
    SWIZZLE_DEBUG_INFO("SwizzleManager::Swizzle() - Requested remap for \"%s\" (0x%08X) in %s.\n", var, id, func);
#endif
...
/**
 *  ViniferaSwizzleManagerClass::Fetch_Swizzle_ID_Dbg
 * 
...
/**
 *  ViniferaSwizzleManagerClass::Here_I_Am_Dbg
 * 
