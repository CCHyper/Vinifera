...
**    animation.
*/
char NewSidebarClass::StripClass::ClockTranslucentTable[(1+1)*256];
...
**    repair buttons.
*/
//TheaterType NewSidebarClass::StripClass::LastTheater = THEATER_NONE;
...
ShapeButtonClass NewSidebarClass::Zoom;
...
//    Column[1].Y = COLUMN_TWO_Y;
    ...
    */
    if (SidebarShape == nullptr) {
        SidebarShape = (void*)MFCC::Retrieve("SIDEBAR.SHP");
    }
}
...
void NewSidebarClass::Init_IO()
{
    ...
    */
    if (!Debug_Map) {
        ...
        Repair.Set_Shape(MFCC::Retrieve("REPAIR.SHP"));
        ...
        Upgrade.Set_Shape(MFCC::Retrieve("SELL.SHP"));
        ...
        Zoom.Set_Shape(MFCC::Retrieve("MAP.SHP"));
 ...
 *   12/24/1994 JLB : Created.                                                                 *
 *=============================================================================================*/
void NewSidebarClass::Init_Theater(TheaterType theater)
{
    Reload_Sidebar();
...
/***********************************************************************************************
 * NewSidebarClass::Reload_Sidebar -- Loads appropriate sidebar shapes depending on house              *
 *                                                                                             *
 * INPUT:  none                                                                                                           *
 *                                                                                             *
 * OUTPUT:  none                                                                               *
 ...
 *=============================================================================================*/
void NewSidebarClass::Reload_Sidebar()
{
    ...
    sb_name[4] = '1';
    SidebarShape = (void*)MFCC::Retrieve(sb_name);
    sb_name[4] = '2';
    SidebarMiddleShape = (void*)MFCC::Retrieve(sb_name);
    sb_name[4] = '3';
    SidebarBottomShape = (void*)MFCC::Retrieve(sb_name);
 ...
 *=============================================================================================*/
int NewSidebarClass::Which_Column(RTTIType type)
{
 ...
 *=============================================================================================*/
bool NewSidebarClass::Factory_Link(int factory, RTTIType type, int id)
{
 ...
 *   12/24/1994 JLB : Created.                                                                 *
 *=============================================================================================*/
bool NewSidebarClass::Activate_Repair(int control)
{
 ...
 *   12/24/1994 JLB : Created.                                                                 *
 *=============================================================================================*/
bool NewSidebarClass::Activate_Upgrade(int control)
{
 ...
 *   12/24/1994 JLB : Created.                                                                 *
 *=============================================================================================*/
bool NewSidebarClass::Activate_Demolish(int control)
{
 ...
 *=============================================================================================*/
void NewSidebarClass::AI(KeyNumType & input, int x, int y)
{
    bool redraw = false;
 ...
 *=============================================================================================*/
NewSidebarClass::StripClass::StripClass(InitClass const & ) :
    X(0),
    Y(0),
    ID(0),
    IsToRedraw(true),
 ...
 *   12/31/1994 JLB : Created.                                                                 *
 *=============================================================================================*/
void NewSidebarClass::StripClass::One_Time(int )
{
    /*
    ** Sidebar is player team specific in Hires
    */
    ClockShapes = MFCC::Retrieve("CLOCK.SHP");

    for (SpecialWeaponType lp = SPECIAL_FIRST; lp < SPECIAL_COUNT; lp++) {
        char buffer[_MAX_FNAME];
        sprintf(buffer, "%sICON", SpecialWeaponFile[lp]);

        char fullname[_MAX_FNAME+_MAX_EXT];
        _makepath(fullname, nullptr, nullptr, buffer, ".SHP");
        SpecialShapes[lp] = MFCC::Retrieve(fullname);
    }
}
 ...
 *=============================================================================================*/
void const * NewSidebarClass::StripClass::Get_Special_Cameo(SpecialWeaponType type)
{
 ...
 *   12/24/1994 JLB : Created.                                                                 *
 *=============================================================================================*/
void NewSidebarClass::StripClass::Init_IO(int id)
{
    ID = id;

    UpButton[ID].IsSticky = true;
    UpButton[ID].ID = BUTTON_UP+id;
    UpButton[ID].X = X+(UP_X_OFFSET);
    UpButton[ID].Y = Y+(UP_Y_OFFSET);

    UpButton[ID].Set_Shape((ShapeFileStruct *)MFCC::Retrieve("STRIPUP.SHP"));

    DownButton[ID].IsSticky = true;
    DownButton[ID].ID = BUTTON_DOWN+id;
    DownButton[ID].X = X+(DOWN_X_OFFSET);
    DownButton[ID].Y = Y+(DOWN_Y_OFFSET);
    ...
    */
    ...
    DownButton[ID].Set_Shape((ShapeFileStruct *)MFCC::Retrieve("STRIPDN.SHP"));

    for (int index = 0; index < MAX_VISIBLE; ++index) {
        SelectClass & g = SelectButton[ID][index];
        g.ID = BUTTON_SELECT;
        g.X = X;
        g.Y = Y + ((OBJECT_HEIGHT*index));
 ...
 *   12/24/1994 JLB : Created.                                                                 *
 *=============================================================================================*/
void NewSidebarClass::StripClass::Init_Theater(TheaterType theater)
{
...
void NewSidebarClass::StripClass::Reload_LogoShapes()
{
    /*
    ...
    */
    ...
    LogoShapes = (void *)MFCC::Retrieve(stripnames[houseloaded]);
}
 ...
 *   12/31/1994 JLB : Uses mouse coordinate parameters.                                        *
 *=============================================================================================*/
bool NewSidebarClass::StripClass::AI(KeyNumType & input, int , int )
{
    bool redraw = false;
    ...
    */
    if (IsBuilding) {
        for (int index = 0; index < BuildableCount; ++index) {
            ...
            if (factoryid != -1) {
                FactoryClass * factory = Factories[factoryid];
                    ...
                    if (obj != nullptr) {
                        ...
                        */
                        ...
                        if (Buildables[index].Factory != -1) {
                            factory         = Factories[Buildables[index].Factory];
                            production    = true;
 ...
 *   05/18/1995 JLB : Created.                                                                 *
 *=============================================================================================*/
bool NewSidebarClass::StripClass::Factory_Link(int factory, RTTIType type, int id)
{
    for (int index = 0; index < BuildableCount; ++index) {
        if (Buildables[index].BuildableType == type && Buildables[index].BuildableID == id) {
 ...
 *   05/18/1995 JLB : Created.                                                                 *
 *=============================================================================================*/
bool NewSidebarClass::Abandon_Production(RTTIType type, int factory)
{
 ...
 *=============================================================================================*/
bool NewSidebarClass::StripClass::Abandon_Production(int factory)
{
    bool noprod = true;
    bool abandon = false;
    for (int index = 0; index < BuildableCount; ++index) {
        if (Buildables[index].Factory == factory) {
            Factories[factory]->Abandon();
            Buildables[index].Factory = -1;
            abandon = true;
        } else {
            if (Buildables[index].Factory != -1) {
