 ...
 *
 ******************************************************************************/
#include "openal_ahandle.h"
#include "openal_audio.h"
#include "debughandler.h"


OpenAL_AHandle AudioHandles[MAX_AUDIO_HANDLES];
...
//0x004072D0
int __stdcall OpenAL_AHandle::func_4072D0(VQAHandle *handle, void *a2)
{
    return 0;
}
...
//0x00407450
int __stdcall OpenAL_AHandle::Stream_Handler(VQAHandle *handle, int action, void *buffer, int nbytes)
{
    int error = 1;
...
// Start Audio?
//0x00407650
int OpenAL_AHandle::func_407650(VQAHandle *handle, void *buffer, int nbytes)
{
#if 0
    DEBUG_INFO("OpenAL: Opening VQ audio handler\n");

    DEBUG_INFO("OpenAL: Current thread ID is %08x\n", GetCurrentThreadId());
    ...
    if (Audio.Is_Available()) {
        ...
        if ( free_index? != 1 && a3 == 16 ) {
            ...
            OpenAL_AHandle &ah = AudioHandles[handle->_audio_handle];

            ZeroMemory(&ah, sizeof(OpenAL_AHandle));
            ...
            //DEBUG_INFO("OpenAL: Audio.Lock_Mutex\n");
            //Audio.Lock_Mutex();
        }
    }
#endif
...
//0x00407980
int OpenAL_AHandle::func_407980(VQAHandle *handle)
{
#if 0
    DEBUG_INFO("OpenAL: Closing VQ audio handler\n");

    OpenAL_AHandle &ah = AudioHandles[handle->_audio_handle];

    if (ah.field_0) {

        DEBUG_INFO("OpenAL: Stop_Audio_Handler\n");
        if (ah.BufferPtr) {
        ...
        DEBUG_INFO("OpenAL: Calling timeKillEvent\n");
        timeKillEvent(ah.TimerHandle);

        ah.TimerHandle = 0;

        DEBUG_INFO("OpenAL: Calling timeEndPeriod\n");
        timeEndPeriod(16);

        ah.field_0 = false;

        if (ah.field_BC) {
            DEBUG_INFO("OpenAL: Changing primary buffer format back to original\n");
            ah.field_BC = false;
            //DEBUG_INFO("OpenAL: Audio.Lock_Mutex()\n");
            //Audio.Lock_Mutex(&Audio);
        }

        DEBUG_INFO("OpenAL: Deleting the critical section object\n");
        DeleteCriticalSection(&ah.CriticalSection);
    }

    DEBUG_INFO("OpenAL: VQ audio handler closed OK\n");
#endif
    return VQAERR_NONE;
}
...
//0x00407B20
int OpenAL_AHandle::func_407B20(VQAHandle *handle)
{
#if 0
    OpenAL_AHandle &ah = AudioHandles[handle->_audio_handle];
...
//0x00407D30
int OpenAL_AHandle::func_407D30(VQAHandle *handle, void *buffer, int nbytes)
{
    return VQAERR_NONE;
}
...
// Stop?
//0x00407EF0
int OpenAL_AHandle::func_407EF0(VQAHandle *handle)
{
#if 0
    OpenAL_AHandle &ah = AudioHandles[handle->_audio_handle];
    EnterCriticalSection(&ah.CriticalSection);
...
//0x00407F40
int OpenAL_AHandle::func_407F40(VQAHandle *handle)
{
    return VQAERR_NONE;
}
...
//0x0407FE0
int OpenAL_AHandle::Stop_Audio_Handler(VQAHandle *vqhandle)
{
    return VQAERR_NONE;
}
...
// NOT NEEDED
//0x00408060
void __stdcall OpenAL_AHandle::SoundTimerCallback(UINT uID, UINT uMsg, DWORD_PTR dwUser, DWORD_PTR dw1, DWORD_PTR dw2)
{
}
...
// NOT NEEDED
//0x00408200
unsigned int OpenAL_AHandle::func_408200()
{
    return 0;
}
...
// Pause Audio?
//0x00408210
void OpenAL_AHandle::func_408210()
{
    OpenAL_AHandle &ah = AudioHandles[0];
...
// Resume Audio?
//0x00408260
void OpenAL_AHandle::func_408260()
{
    OpenAL_AHandle &ah = AudioHandles[0];
...
// Open Audio?
//0x004082B0
int OpenAL_AHandle::func_4082B0()
{
    return 1;
}
...
// Close Audio?
//0x004082C0
int OpenAL_AHandle::func_4082C0()
{
    return 1;
}
