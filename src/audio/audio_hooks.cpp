 ...
 *  a static function.
 */
static bool __cdecl Audio_Driver_Init(HWND hWnd, int bits_per_sample, bool stereo, int rate, int num_trackers) { return Direct_Sound_Init() && Audio_Driver()->Init(hWnd, bits_per_sample, stereo, rate, num_trackers); }
static void __cdecl Audio_Driver_End() { Direct_Sound_End(); Audio_Driver()->End(); }
static bool __cdecl Audio_Driver_Is_Available() { return Direct_Sound_Is_Available() && Audio_Driver()->Is_Available(); }
static bool __cdecl Audio_Driver_Is_Enabled() { return Audio_Driver()->Is_Enabled(); }
static void __cdecl Audio_Driver_Enabled() { Audio_Driver()->Enable(); }
static void __cdecl Audio_Driver_Disable() { Audio_Driver()->Disable(); }
static void __cdecl Audio_Driver_Stop_Sample(int handle) { return Audio_Driver()->Stop_Sound_Effect((AudioHandle)handle); }
static bool __cdecl Audio_Driver_Sample_Status(int handle) { return Audio_Driver()->Is_Sound_Effect_Playing((AudioHandle)handle); }

static bool __cdecl Audio_Driver_Is_Sample_Playing(const void *sample)
{
    
    //return Audio_Driver()->Is_Sample_Playing((AudioSample)sample);
}

static void __cdecl Audio_Driver_Stop_Sample_Playing(const void *sample)
{
    //Audio_Driver()->Stop_Sample_Playing((AudioSample)sample);
}

static int __cdecl Audio_Driver_Play_Sample(const void *sample, int priority = PRIORITY_MAX, int volume = VOLUME_MAX) { return (int)Audio_Driver()->Play_Sample((AudioSample)sample, priority, volume); }

static int __cdecl Audio_Driver_File_Play_Sample(const char *filename, int priority = PRIORITY_MAX, int volume = VOLUME_MAX) { return (int)Audio_Driver()->Play_File(Wstring(filename), priority, volume); }

static int __cdecl Audio_Driver_File_Stream_Sample_Vol(const char *filename, int volume, bool real_time_start = false) { return (int)Audio_Driver()->Stream_File(Wstring(filename), volume, real_time_start); }

static void __cdecl Audio_Driver_Fade_Sample(int handle, int ticks = 60) { Audio_Driver()->Fade_Sound_Effect((AudioHandle)handle, ticks); }
static void __cdecl Audio_Driver_Set_Handle_Volume(int handle, int volume) { Audio_Driver()->Set_Sound_Effect_Volume((AudioHandle)handle, volume); }
static void __cdecl Audio_Driver_Set_Sample_Volume(const void *sample, int volume) { Audio_Driver()->Set_Sample_Volume((AudioSample)sample, volume); }
static int __cdecl Audio_Driver_Set_Sound_Vol(int volume) { return Audio_Driver()->Set_All_Sound_Effect_Volume(volume); }
static int __cdecl Audio_Driver_Set_Score_Vol(int volume) { return Audio_Driver()->Set_Music_Volume(volume); }
static void __cdecl Audio_Driver_Set_Volume_All(int volume) { Audio_Driver()->Set_Volume_All(volume); }
static int __cdecl Audio_Driver_Set_Volume_Percent_All(int vol_percent) { return Audio_Driver()->Set_Master_Volume(vol_percent); }
static bool __cdecl Audio_Driver_Start_Primary_Sound_Buffer(bool forced = false) { return Direct_Sound_Start() && Audio_Driver()->Resume(forced); }
static void __cdecl Audio_Driver_Stop_Primary_Sound_Buffer() { Direct_Sound_Stop(); Audio_Driver()->Suspend(); }
static void __cdecl Audio_Driver_Sound_Callback() { Audio_Driver()->Sound_Callback(); }
//static int __cdecl Audio_Driver_Play_Sample_Handle(const void *sample, int priority, int volume, int handle) { return (int)Audio_Driver()->Play_Sample_Handle((AudioSample)sample, priority, volume, (AudioHandle)handle); }
static void __cdecl Audio_Driver_Set_Focus_Loss_Function(void (*func)()) { Audio_Driver()->Set_Focus_Loss_Function(func); }
static void __cdecl Audio_Driver_Set_Stream_Low_Impact(bool set) { Audio_Driver()->Set_Stream_Low_Impact(set); }


/**
 *  Handy macros for defining the audio engine patches.
 * 
 *  @author: CCHyper
 */
#define AUDIO_ENGINE_INIT_PATCH(label, ret_addr) \
    ...
    { \
        _asm { pushad } \
        Audio_Driver_Init(MainWindow, AUDIO_DEFAULT_BITS, AUDIO_DEFAULT_STEREO, AUDIO_DEFAULT_RATE, Audio_MaxSampleTrackers); \
        _asm { popad } \
        JMP(ret_addr); \
    }
