...
void Vinifera_Swizzle_Request_Pointer_Remap_Vector(DynamicVectorClass<T> &vector, const char *file, const int line, const char *func, const char *var)
{
    ...
    for (int i = 0; i < vector.Count(); ++i) {
        ViniferaSwizzleManager.Swizzle_Dbg((void **)&vector[i], file, line, func, var);
    }
#else
    for (int i = 0; i < vector.Count(); ++i) {
...
#define VINIFERA_SWIZZLE_HERE_I_AM(id, pointer, variable)                 Vinifera_Swizzle_Here_I_Am(id, (void *)pointer, __FILE__, __LINE__, __FUNCTION__ "()", variable);
