...
/**
 *  CnCNet5KCPInterfaceClass constructor.
 * 
 *  @author: CCHyper
 */
CnCNet5KCPInterfaceClass::CnCNet5KCPInterfaceClass(unsigned short id, unsigned long ip, unsigned short port, bool port_hack, int kcp_session_num) :
    UDPInterfaceClass(),
    IsEnabled(false),
    AddressList(),
    TunnelID(id),
    ...
    PortHack(port_hack),
    KCPObject(nullptr),
    KCPSessionNumber(kcp_session_num)
{
    ...
    KCPObject = ikcp_create(kcp_session_num, &kcp_callback);
    ASSERT_FATAL(KCPObject != nullptr);

    // The call will set the maximum send window and maximum receive window size of the
    // procotol, which is 32 by default. This can be understood as SND_BUF and RCV_BUF
    // of TCP, but the unit is not the same, SND / RCV_BUF unit is byte, while this unit
    // is the packet.
    ikcp_wndsize(KCPObject, 128, 128);

    // Set the callback function
    KCPObject->output = kcp_udp_output;
}
 ...
 *  @author: CCHyper
 */
LRESULT CnCNet5KCPInterfaceClass::Message_Handler(HWND hWnd, UINT uMsg, UINT wParam, LONG lParam)
{
    /**
    ...
    switch (WSAGETSELECTEVENT(lParam)) {
        ...
        case FD_READ:
            ...
            rc = CnCNet5KCPInterfaceClass::Receive_From(Socket, (char*)ReceiveBuffer, sizeof(ReceiveBuffer), 0, (PSOCKADDR_IN)&addr, &addr_len);
            if (rc == SOCKET_ERROR) {
                DEBUG_WARNING("CnCNet5[KCP]: Send_To returned %d!\n", WSAGetLastError());
                Clear_Socket_Error(Socket);
                return 0;
            }
            ...
            if (rc != 0) {
                ...
                if (!Passes_CRC_Check(packet)) {
                    DEBUG_INFO("CnCNet5[KCP]: Throwing away malformed packet!\n");
        ...
        case FD_WRITE:
             ...
             */
            rc = CnCNet5KCPInterfaceClass::Send_To(Socket, (const char *)&packet->PacketData, packet->BufferLen, 0, (PSOCKADDR_IN)&addr, sizeof (addr));
            if (rc == SOCKET_ERROR){
 ...
 */
int CnCNet5KCPInterfaceClass::Send_To(SOCKET s, const char *buf, int len, int flags, sockaddr_in *dest_addr, int addrlen)
{
    char tempbuf[1024 + 4];
     ...
     */
    if (TunnelPort == -1) {
        DEBUG_WARNING("CnCNet5[KCP]: TunnelPort is invalid in Send_To!\n");
        return ikcp_send(s, buf, len, flags, (const sockaddr *)dest_addr, addrlen);
    }

#ifndef NDEBUG
    //DEV_DEBUG_INFO("CnCNet5[KCP]: ikcp_send(s=%d, buf=%p, len=%d, flags=%08X, to=%p, addrlen=%d)\n", s, buf, len, flags, dest_addr, addrlen);
#endif
    ...
    *bufto = dest_addr->sin_port;
    ...
    return ikcp_send(s, tempbuf, len + 4, flags, (const sockaddr *)dest_addr, addrlen);
}
 ...
 *  @author: CCHyper (based on implementation by Toni Spets).
 */
int CnCNet5KCPInterfaceClass::Receive_From(SOCKET s, char *buf, int len, int flags, sockaddr_in *src_addr, int *addrlen)
{
    char tempbuf[1024 + 4];
    unsigned short *buffrom = reinterpret_cast<unsigned short *>(&tempbuf[0]);
    unsigned short *bufto = reinterpret_cast<unsigned short *>(&tempbuf[2]);
     ...
     */
    if (TunnelPort == -1) {
        DEBUG_WARNING("CnCNet5[KCP]: TunnelPort is invalid in Recieve_From!\n");
        return ikcp_recv(s, buf, len, flags, (sockaddr *)src_addr, addrlen);
    }

#ifndef NDEBUG
    //DEV_DEBUG_INFO("CnCNet5[KCP]: ikcp_recv(s=%d, buf=%p, len=%d, flags=%08X, from=%p, addrlen=%p (%d))\n", s, buf, len, flags, src_addr, addrlen, *addrlen);
#endif

    /**
     *  Call recvfrom first to get the packet.
     */
    int ret = ikcp_recv(s, tempbuf, sizeof tempbuf, flags, (sockaddr *)src_addr, addrlen);

    /**
     *  No processing if returning error or less than 5 bytes of data.
     */
    if (ret < 5 || *bufto != TunnelID) {
        DEBUG_WARNING("CnCNet5[KCP]: ikcp_recv returned invalid data!\n");
        return -1;
    }
